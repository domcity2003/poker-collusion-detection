...
def main():
 ...
 for tag in ['baseline','context']:
  t=time.time();cols=[c for c in fc if tag!='baseline' or not c.startswith('cx_')]
  path=out/f'{tag}.npy'
  if path.exists():scores=np.load(path)
   ...
   for f in [1,2,3,4]:
    ...
    for k,fm in enumerate(FAM):
     ...
     m=CatBoostRanker(loss_function='YetiRank',iterations=600,depth=5,learning_rate=.06,thread_count=5,verbose=False,random_seed=2026,allow_writing_files=False)
  ...
  preds[tag]=rank
 for tag,rank in [('baseline',preds['baseline']),('context',preds['context']),('blend50',.5*preds['context']+.5*preds['baseline'])]:
  s=np.zeros(len(y))
 ...
 Path('reports/context_experiment.json').write_text(json.dumps(res,indent=2))
